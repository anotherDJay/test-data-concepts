...
class SnowflakeClient:
    ...
    def load_two_weeks_data(
        ...
        """
        Load two weeks of energy data (current + previous) in a single optimized query.
        ...
        local_tz = ZoneInfo(tz_str)
        current_start = datetime.combine(current_week_start, datetime.min.time()).replace(tzinfo=local_tz)
        current_end = current_start + timedelta(days=7)
        prev_start = current_start - timedelta(days=7)

        # Convert to naive UTC
        prev_start_utc = prev_start.astimezone(ZoneInfo("UTC")).replace(tzinfo=None)
        current_end_utc = current_end.astimezone(ZoneInfo("UTC")).replace(tzinfo=None)

        try:
            # Fetch consumption and production for both weeks in one query each
            cons = (
                self.session
                .table("ENERGY_SHARED.TRUNKS_HELIOS.HOURLY_CONSUMPTION")
                .filter(
                    (col("HELIOS_SITE_ID") == site_id)
                    & (col("CONSUMPTION_ON") >= lit(prev_start_utc))
                    & (col("CONSUMPTION_ON") < lit(current_end_utc))
                )
                .select(
                    col("CONSUMPTION_ON").alias("ts"),
                    col("CONSUMPTION_KWH").alias("cons_kwh"),
            ...
            prod = (
                ...
                .filter(
                    (col("HELIOS_SITE_ID") == site_id)
                    & (col("PRODUCTION_ON") >= lit(prev_start_utc))
                    & (col("PRODUCTION_ON") < lit(current_end_utc))
                )
                .select(
                    col("PRODUCTION_ON").alias("ts"),
                    col("PRODUCTION_KWH").alias("prod_kwh"),
            ...
            raise Exception(f"Error loading two weeks data: {e}")
        ...
        week_boundary = current_start.astimezone(ZoneInfo(tz_str))
        df_current = df[df["ts"] >= week_boundary].copy().reset_index(drop=True)
        df_prev = df[df["ts"] < week_boundary].copy().reset_index(drop=True)

        return (
            df_current if not df_current.empty else None,
            df_prev if not df_prev.empty else None
        )
